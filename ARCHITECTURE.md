...
## 🚀 Dağıtım ve Altyapı Bilgileri

### 1. CI/CD Süreci
* **GitHub Actions:** Proje test ve dağıtım süreçleri GitHub Actions iş akışları ile otomatik olarak yürütülmektedir. Koda yapılan her değişiklik GitHub'a push yapıldığında GitHub Actions tetiklenir, otomatik derlenir (build edilir) ve uzak sunucuya (`192.168.1.116`) dağıtılır. Bu nedenle yerel ortamda derlenen binary dosyalarının manuel olarak taşınmasına veya yüklenmesine gerek yoktur.

### 2. Uzak Sunucu Bilgileri (Remote Server)
* **IP Adresi:** `192.168.1.116`
* **Sunucu Adı (Hostname):** `vm-ai1`
* **Kullanıcı Adı (User):** `rhod`
* **Çalıştırma Şekli:** Derlenen Linux binary'si ev dizini altında (`~/chirpstack-simulator`) çalıştırılır.
